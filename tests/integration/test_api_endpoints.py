...
class TestVehicleAPI:
    ...
    def test_create_vehicle(self, api_client, db):
        ...
        response = api_client.post(
            "/api/v1/vehicles/", data=data, content_type="application/json"
        )
    ...
    def test_update_vehicle_fcm_token(self, api_client, sample_vehicle):
        """FCM 토큰 업데이트 테스트"""
        data = {"fcm_token": "new-fcm-token-updated"}
...
class TestDetectionAPI:
    ...
    def test_filter_detections_by_status(
        self, api_client, pending_detection, completed_detection
    ):
    ...
    def test_filter_detections_by_camera(self, api_client, sample_detection):
        ...
        response = api_client.get(
            f"/api/v1/detections/?camera_id={sample_detection.camera_id}"
        )
