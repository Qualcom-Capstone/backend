...
class TestOCRTaskMock:
    ...
    def test_process_ocr_mock_mode(self, mock_env, pending_detection):
        ...
        result = process_ocr(pending_detection.id, gcs_uri=pending_detection.image_gcs_uri)
...
class TestNotificationTaskMock:
    ...
    def test_notification_body_format(self, completed_detection):
        """알림 본문 형식 테스트"""
        body = f"📍 위치: {completed_detection.location}\n🚗 속도: {completed_detection.detected_speed}km/h (제한: {completed_detection.speed_limit}km/h)"
